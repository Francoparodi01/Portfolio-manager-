    ...
    def __init__(
        ...
        *,
        ...
        self.ttl_seconds = int(
            ttl_seconds or os.getenv("QUANTIA_CONVERSATION_TTL_SECONDS", "86400")
        )
        ...
        *,
        ...
        if self.run_store:
            try:
                ...
                recent = await self.run_store.recent_context(
                    owner_chat_id,
                    namespace="conversational-harness",
                )
            except Exception:
                recent = []
    ...
    async def save(self, state: ConversationState) -> None:
        ...
        try:
            await redis_client.set(
                self._key(state.owner_chat_id),
                state.model_dump_json(),
                ex=self.ttl_seconds,
            )
        except Exception:
            # Audit metadata remains the durable trace. Redis loss degrades
