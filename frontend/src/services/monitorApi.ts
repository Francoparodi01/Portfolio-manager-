...
export function endpointDefinitions(period: number): EndpointDefinition[] {
  return [
    ...
    { key: "timeline", label: "Timeline", path: `/api/audit-timeline?days=${period}&limit=400`, timeoutMs: 35_000 },
...
export const monitorApi = {
  ...
  timeline: (session: ApiSession, days = 90, limit = 400) => fetchJson<AuditTimelinePayload>(session, `/api/audit-timeline?days=${days}&limit=${limit}`, 35_000),
