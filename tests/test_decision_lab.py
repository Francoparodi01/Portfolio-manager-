...
def dataset_fixture(*, quantity=10, cash=1000, sell=4):
    portfolio = ev("PORTFOLIO", {"cash_ars":cash, "positions":[{"ticker":"AAA","quantity":quantity,"mark_ars":100,"lot_size":1}]}, owner=123,rid="portfolio")
    plan = ev("PLAN", {"complete":True,"cash_before":cash,"feasible":True,"portfolio_snapshot_id":"portfolio",
        "orders":[{"ticker":"AAA","side":"SELL","quantity":sell,"reference_price":100,"target_amount_ars":sell*100,
                   "action":"SELL_PARTIAL","executable":True,"current_weight":.5,"target_weight":.3}]},owner=123,rid="plan")
    sessions=[]; bars=[]
    for i in range(1,45):
        opened=T+timedelta(days=i,hours=-7)
        session=Session(session_id=f"s{i}",open_at=opened,close_at=opened+timedelta(hours=6))
        sessions.append(session)
        bars.append(ev("BAR",{"ticker":"AAA","session_id":session.session_id,"open":100,"high":125,"low":90,"close":100+i,
                               "volume":1000,"price_mode":"RAW_AS_TRADED"},at=session.close_at))
    return Dataset(records=(portfolio,plan,*bars),sessions=tuple(sessions),calendar_version="test-calendar")


def recorded_plan(dataset):
    state=build_state(EvidenceIndex(dataset),as_of=T,owner=123,plan_id="plan")
    spec=current_spec("recorded_plan_v1",registered_at=T,code_version="fixture",config_hash="fixture")
...
def test_future_snapshots_universe_news_and_revision_do_not_change_state():
    ...
    future=(ev("PORTFOLIO",{"cash_ars":9999,"positions":[]},at=T+timedelta(days=1),owner=123),
            ev("UNIVERSE",{"instruments":[{"ticker":"FUTUREWINNER","enabled":True,"operable":True}]},available=T+timedelta(seconds=1)),
            ev("NEWS",{"published_at":(T+timedelta(seconds=1)).isoformat(),"headline":"future outcome"}),
            ev("MACRO",{"vix":80},at=T-timedelta(days=1),revision=T+timedelta(days=1)))
    changed,_=recorded_plan(data.model_copy(update={"records":data.records+future}))
...
def test_recorded_plan_preserves_quantity_and_is_not_historical_strategy_replay():
    ...
    with pytest.raises(InsufficientEvidence,match="RUNTIME_DIFFERS"):
        freeze_plan(state,plan.strategy.model_copy(update={"implementation_hash":"wrong"}),"RECORDED_PLAN_EVALUATION")


def test_modern_policy_cannot_be_backdated_as_historical():
    state,_=recorded_plan(dataset_fixture())
    spec=current_spec("hold_v1",registered_at=T+timedelta(days=10),code_version="fixture",config_hash="fixture")
    ...
    with pytest.raises(InsufficientEvidence,match="TRAINING"):
        freeze_plan(state,spec.model_copy(update={"training_end":T}),"CURRENT_POLICY_ON_HISTORICAL_DATA")


from src.decision_lab.models import CostModel
from src.decision_lab.counterfactuals import freeze_episode, evaluate_episode, comparisons


def run_fixture(data=None, costs=None, cutoff=None):
    data=data or dataset_fixture()
    state,plan=recorded_plan(data)
    episode=freeze_episode(state,plan,costs or CostModel(),Experiment(experiment_id="fixture",registered_at=T))
    outcomes=evaluate_episode(episode,EvidenceIndex(data),evaluated_as_of=cutoff or T+timedelta(days=45))
...
def test_manual_common_capital_hold_plan_partial_and_dva():
    ...
    assert rows["HOLD"].net_return==pytest.approx(.025)
    assert json.loads(rows["HOLD"].ending_positions_json)=={"AAA":"10"}
    assert rows["HOLD"].executed_orders_json=="[]"
    assert rows["PLAN"].gross_return==pytest.approx(.015)
    assert rows["PLAN"].net_return==pytest.approx(.0135)
    assert rows["PARTIAL_50"].net_return==pytest.approx(.01925)
    assert rows["CASH"].net_return==pytest.approx(-.00375)
    assert comparisons(episode,outcomes)[0]["dva"]==pytest.approx(-.0115)
...
def test_missing_and_pending_are_never_zero_returns():
    ...
    assert next(o for o in rows if o.horizon==40 and o.alternative=="PLAN").status=="PENDING"
    data=data.model_copy(update={"records":tuple(e for e in data.records if not(e.kind=="BAR" and e.payload["session_id"]=="s5"))})
...
def test_future_bars_do_not_change_frozen_outputs_or_hashes():
    ...
    before=data.model_copy(update={"records":tuple(e for e in data.records if e.available_at<=cutoff)})
...
def test_split_raw_prices_quantity_and_no_double_adjustment():
    ...
    for e in data.records:
        ...
        records.append(e)
    records.append(ev("CORPORATE_ACTION",{"ticker":"AAA","type":"SPLIT","quantity_factor":2},at=T+timedelta(hours=1)))
    _,rows=run_fixture(data.model_copy(update={"records":tuple(records)}),costs=CostModel(fee_bps=0))
    row=next(o for o in rows if o.horizon==5 and o.alternative=="HOLD")
    assert row.net_return==0 and json.loads(row.ending_positions_json)=={"AAA":"20"}
    row=next(o for o in rows if o.horizon==5 and o.alternative=="PLAN")
...
def test_best_candidate_uses_frozen_ranking_not_future_winner():
    data=dataset_fixture()
    universe=ev("UNIVERSE",{"universe_quality":"EXACT","instruments":[
        {"ticker":"BBB","enabled":True,"operable":True,"candidate_score":.8,"reference_price_ars":10,"lot_size":1},
        {"ticker":"CCC","enabled":True,"operable":True,"candidate_score":.1,"reference_price_ars":10,"lot_size":1}]})
    episode,_=run_fixture(data.model_copy(update={"records":data.records+(universe,)}))
    best=next(a for a in json.loads(episode.alternatives_json) if a["name"]=="BEST_AVAILABLE_CANDIDATE")
...
def test_real_core_kernels_offline_deterministic(monkeypatch):
    ...
    data=dataset_fixture()
    config={"scope":"PORTFOLIO_ONLY_NO_RADAR","lookback_sessions":70,"sentiment_enabled":False,
            "portfolio_history":[{"scraped_at":T.isoformat(),"total_value_ars":2000}],"events_complete":True}
    records=[e for e in data.records if e.kind!="PLAN"]
    portfolio=records[0].payload
    portfolio["positions"].append({"ticker":"BBB","quantity":5,"mark_ars":100,"lot_size":1})
    records[0]=records[0].model_copy(update={"payload_json":canonical(portfolio)})
    records.extend([ev("CONFIG",config,owner=123),ev("UNIVERSE",{"universe_quality":"EXACT","instruments":[{"ticker":ticker,"enabled":True,"operable":True} for ticker in ("AAA","BBB")]}),
                    ev("MACRO",{"vix":17,"sp500_trend":.02,"ccl":1400,"riesgo_pais":500})])
    sessions=list(data.sessions)
    for i in range(70):
        close=T-timedelta(days=70-i)
        session=Session(session_id=f"past{i}",open_at=close-timedelta(hours=6),close_at=close)
        sessions.append(session)
        price=80+i*.3+(i%3)
        records.append(ev("BAR",{"ticker":"AAA","session_id":session.session_id,"open":price,"high":price+1,"low":price-1,
                                 "close":price,"volume":10000,"price_mode":"RAW_AS_TRADED"},at=close))
    ...
    state=build_state(EvidenceIndex(Dataset(records=tuple(records),sessions=tuple(sessions),calendar_version="fixture")),as_of=T,owner=123)
    spec=current_spec("quantia_core_v1",registered_at=T,code_version="fixture",config_hash=digest(config))
