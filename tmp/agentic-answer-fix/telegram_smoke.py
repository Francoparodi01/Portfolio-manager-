...
class Sink:
    ...
    async def send_document(self, **kwargs):
        ...
        Path('/verification/telegram-answer-smoke.json').write_text(json.dumps(self.result,ensure_ascii=False,indent=2),encoding='utf-8')

async def main():
    cfg=get_config(); owner=int(cfg.scraper.telegram_chat_id)
    sink=Sink()
    bot.ensure_allowed_chat=AsyncMock(return_value=True)
    goal='revisa mi cartera y explica que evidencia falta para decidir'
    ...
    assert result['answer_origin']=='evidence_renderer_v1'
    assert 'get_portfolio_snapshot' in result['answer']
    assert 'Resumen:' in result['answer'] and 'Faltantes y límites:' in result['answer']
    assert 'no valida la conclusión' in '\n'.join(sink.messages)
    assert 'Consultas:' in '\n'.join(sink.messages)
    assert set(Path(tempfile.gettempdir()).glob('quantia_agent_*'))==before
    conn=await connect_read_only(cfg.database.url)
    try:
        run=await conn.fetchrow('SELECT status, owner_chat_id, metadata FROM agent_runs WHERE id=$1::uuid',result['run_id'])
        ...
        metadata=json.loads(run['metadata']) if isinstance(run['metadata'],str) else run['metadata']
        assert metadata['answer_origin']=='evidence_renderer_v1'
