...
BOT_COMMAND_SPECS: list[tuple[str, str]] = [
    ...
    ("seguimiento", "Evolución de ideas seguidas"),
    ...
    ("analytics", "Analytics v2: episodios, costos y evidencia"),
...
async def sync_operational_state(
    *,
    ...
    if result.get("ok"):
        ...
        return ""
    error = str(result.get("error") or "refresh_channel_failed")
    retry_after = int(result.get("retry_after_seconds") or 0)
    if error == "cocos_access_blocked":
        detail = (
            "Cloudflare está bloqueando el acceso al login de Cocos. "
            "No es un rechazo de usuario o contraseña."
        )
    elif error == "cocos_access_cooldown":
        wait_text = f" ({retry_after // 60 + 1} min aprox.)" if retry_after else ""
        detail = f"Cocos está en pausa preventiva después del bloqueo{wait_text}."
    elif error == "cocos_authentication_failed":
        detail = "Cocos no completó la autenticación de la sesión persistente."
    elif error == "persistent_session_not_owned_by_user":
        detail = "La sesión persistente pertenece a otra cuenta configurada."
    elif error == "scheduler_refresh_timeout":
        detail = "El scheduler no respondió a tiempo al pedido de actualización."
    else:
        detail = str(result.get("detail") or error)
    return (
...
def main_keyboard() -> InlineKeyboardMarkup:
    ...
    rows = [
        ...
        [
            InlineKeyboardButton("👁 Seguimiento", callback_data="followed_watchlist"),
        ],
        [
...
def audit_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📐 Analytics v2", callback_data="analytics_v2")],
        [
...
def help_text() -> str:
    return (
        ...
        "<code>/seguimiento</code>: evolución de las ideas que marcaste para seguir.\n"
        ...
        "<code>/analytics</code>: Analytics v2 observacional, con episodios y archivo auditable.\n"
...
async def _load_portfolio_daily_positions(
    db,
    snapshot: dict,
    *,
    reference_at: datetime,
) -> dict[str, dict]:
    """Calculate daily marks with the same freshness/event guards as opening."""
    ...
    for row in await db.get_latest_market_prices():
        ...
        if math.isfinite(price) and price > 0 and ts and ts <= reference_at:
            prices.append(dict(row))
    previous_closes = await db.get_previous_candle_closes(
        tickers, before_day=reference_at.astimezone(TZ).date(),
    )
    for row in prices:
        previous_close = previous_closes.get(str(row.get("ticker") or "").upper())
        if previous_close:
            row["previous_close_price"] = previous_close
    effects = await db.get_corporate_action_effects(tickers=tickers)
    live = build_live_portfolio(
        snapshot, prices, generated_at=reference_at, corporate_action_effects=effects,
    )
    return {p["ticker"]: p for p in live["positions"]}


def _portfolio_daily_line(position: dict) -> str:
    if position.get("price_quality_status") in {"PRICE_NOT_COMPARABLE", "DATA_QUALITY_BLOCK"}:
        return "Hoy: <b>N/A</b> · precio no comparable"
    if position.get("price_source") != "market_prices":
        return "Hoy: <b>N/A</b> · sin cotización de hoy"
    change = position.get("change_pct_1d")
    amount = position.get("day_pnl_ars")
    if change is None or amount is None:
        return "Hoy: <b>N/A</b> · sin referencia comparable"
    change, amount = float(change), float(amount)
    if not math.isfinite(change) or not math.isfinite(amount):
        return "Hoy: <b>N/A</b> · sin referencia comparable"
    ts = _parse_dt(position.get("market_price_ts"))
    time_label = f" · {ts:%H:%M} ART" if ts else ""
    pct = f"{change:+.2%}".replace(".", ",")
    return f"Hoy: <b>{pct}</b> · <b>{_money_signed(amount)} ARS</b>{time_label}"


def _render_portfolio_positions(
    positions: list[dict],
    total_invested: float,
    *,
    daily_positions: dict[str, dict] | None = None,
) -> str:
    """
    ...
    - Muestra el cambio diario estimado, no el retorno desde la compra.
    ...
    for p in positions:
        ...
        daily = (daily_positions or {}).get(str(p.get("ticker") or "").upper(), {})
        rows.append(_portfolio_daily_line(daily))
...
async def action_portfolio(
    ...
    *,
    ...
    try:
        ...
        if snap:
            try:
                daily_positions = await _load_portfolio_daily_positions(
                    db, snap, reference_at=datetime.now(TZ),
                )
            except Exception:
                logger.exception("[BOT][PORTFOLIO] no pude cargar la variación diaria")
                daily_note = "⚠️ No pude consultar la variación diaria; queda en N/A."
    finally:
        await db.close()
    ...
    #
    # El cambio diario usa precios vs. cierre previo y la cantidad actual.
    # El rendimiento desde la compra queda centralizado en /performance.
    #
    ...
    lines += [
        ...
        "",
        _render_portfolio_positions(positions, total_invested, daily_positions=daily_positions),
        "",
        "<i>Hoy: cambio vs. cierre previo según la última cotización guardada. "
        "ARS: efecto estimado sobre la cantidad actual, sin comisiones; "
        "no es ganancia desde la compra.</i>",
        "",
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
        "<i>Portfolio — Cocos Copilot</i>",
    ]
...
async def action_analytics_v2(context: ContextTypes.DEFAULT_TYPE, chat_id: int) -> None:
    with tempfile.TemporaryDirectory(prefix="quantia_analytics_") as folder:
        archive = Path(folder) / "quantia_analytics_v2_180d.zip"
        rc, out, _err, elapsed = await run_cmd(
            [sys.executable, "scripts/run_analytics_v2.py", "--days", "180",
             "--owner-chat-id", str(chat_id), "--archive-out", str(archive)],
            timeout=240,
        )
        if rc != 0 or not out.strip() or not archive.is_file():
            logger.warning("[BOT][ANALYTICS] Report failed rc=%s elapsed=%.1f", rc, elapsed)
            await send_text(context, chat_id,
                            "<b>No pude completar Analytics v2.</b>\n"
                            "La captura o su validación falló. Podés reintentar con /analytics.")
            return
        await send_text(context, chat_id, out.strip())
        with archive.open("rb") as document:
            await context.bot.send_document(
                chat_id=chat_id, document=document, filename=archive.name,
                caption="Analytics v2 · captura de tu cuenta, episodios, escenarios de costo y manifest. Evidencia observacional.",
            )
...
def _followed_radar_lines(
    ...
    *,
    ...
    for row in followed:
        ...
        progress = _followed_price_change(row)
        if progress is not None:
            details.append(f"desde seguimiento {progress:+.1%}")
...
def _watch_price(value) -> str | None:
    try:
        price = float(value)
...
def _followed_price_change(row: dict) -> float | None:
    try:
        baseline = float(row.get("baseline_price"))
    ...
    if (
        ...
        return None
    baseline_at = _parse_dt(row.get("baseline_price_at"))
    current_at = _parse_dt(row.get("current_price_at"))
    if baseline_at is not None and current_at is not None and current_at < baseline_at:
        return None
    return current / baseline - 1.0


def _followed_setup_level(row: dict) -> str | None:
    current_price = _watch_price(row.get("current_price"))
    if current_price is None:
        return None
    try:
        current = float(row.get("current_price"))
        invalidation = float(row.get("invalidation_price"))
    except (TypeError, ValueError):
        return None
    if not math.isfinite(current) or not math.isfinite(invalidation):
        return None
    if current <= invalidation:
        return "⚠️ último precio bajo la invalidación experimental"
    try:
    ...
    if target is not None and math.isfinite(target) and target > 0:
        if current >= target:
            return "🎯 último precio sobre el objetivo experimental"
        return "↔ último precio entre invalidación y objetivo experimental"
    return "↔ último precio sobre la invalidación experimental"


def render_followed_watchlist(followed: list[dict]) -> str:
    """Render an observational, non-execution view of explicit Radar follows."""
    lines = [
        "👁 <b>Seguimiento de ideas · último precio disponible</b>",
        "<i>Compara el precio al marcar Seguir con el último dato de mercado. No es rendimiento real ni una recomendación.</i>",
    ]
    if not followed:
        lines.extend([
            "",
            "No tenés ideas en seguimiento de los últimos 30 días.",
            "Marcá <b>Seguir</b> en una alerta o idea del Radar para verla acá.",
        ])
        return "\n".join(lines)

    for row in followed:
        ticker = html_text(str(row.get("ticker") or "N/D").upper())
        source = str(row.get("source") or "")
        action_at = _fmt_dt_art(row.get("user_action_at"))
        baseline = _watch_price(row.get("baseline_price"))
        current = _watch_price(row.get("current_price"))
        baseline_source = str(row.get("baseline_source") or "")
        baseline_label = (
            "al marcar Seguir"
            if baseline_source == "market_price_at_follow"
            else "al activarse la alerta"
        )
        lines.extend(["", f"<b>{ticker}</b> · seguido {action_at}"])
        if baseline and current:
            lines.append(
                f"Precio {baseline_label}: <b>{baseline}</b> · último: <b>{current}</b>"
            )
            progress = _followed_price_change(row)
            if progress is not None:
                current_at = _fmt_dt_art(row.get("current_price_at"))
                lines.append(f"Evolución observada: <b>{progress:+.1%}</b> · dato {current_at}")
            else:
                lines.append("Evolución: <b>no comparable</b> · el último precio es anterior a la referencia")
        elif current:
            lines.append(
                f"Último precio: <b>{current}</b> · dato {_fmt_dt_art(row.get('current_price_at'))}"
            )
            lines.append("Evolución: <b>sin precio base suficiente</b>")
        else:
            lines.append("Evolución: <b>sin último precio de mercado disponible</b>")
        ...
        if source == "SETUP_ALERT":
            ...
            setup_bits: list[str] = []
            if setup_score is not None:
                setup_bits.append(f"setup {float(setup_score):.1f}/50")
            if percentile is not None:
                setup_bits.append(f"pct {float(percentile):.0%}")
            if setup_bits:
                lines.append(f"Referencia inicial: {' · '.join(setup_bits)}")
            level = _followed_setup_level(row)
            if level:
                lines.append(f"Nivel del setup: {level}")

        matched = bool(row.get("broker_fill_id"))
        execution = "compra real vinculada; ver /performance" if matched else "sin compra confirmada"
        lines.append(f"Estado: {execution}")

    lines.extend([
        "",
        "<i>La variación es sólo de precio, no descuenta comisiones ni representa una operación. Para la lectura técnica vigente: /ticker TICKER.</i>",
        "<i>Este seguimiento no modifica el ranking, V3 ni las métricas prospectivas del Radar.</i>",
    ])
    return "\n".join(lines)
...
def compact_radar_report(
    ...
    *,
    ...
    if detailed:
        lines.extend([
            "<i>Para ver cómo evolucionan las ideas que seguís: /seguimiento.</i>",
...
async def action_followed_watchlist(
    context: ContextTypes.DEFAULT_TYPE,
    chat_id: int,
) -> None:
    try:
        followed = await _load_followed_radar_watchlist(chat_id)
    except Exception:
        logger.exception("[BOT][FOLLOW] no pude cargar progreso de seguimientos")
        await send_text(
            context,
            chat_id,
            "⚠️ No pude leer el seguimiento ahora. Probá de nuevo cuando se actualice la base de mercado.",
        )
        return
    await send_text(context, chat_id, render_followed_watchlist(followed))
...
CALLBACK_ALIASES: dict[str, str] = {
    ...
    "seguimiento":      "followed_watchlist",
    "watchlist":        "followed_watchlist",
    "followed_watchlist": "followed_watchlist",
    # Tesis shadow
    "shadow":           "shadow",
...
ACTION_LOADING_TEXT: dict[str, str] = {
    ...
    "analytics_v2":  "Auditando episodios, costos y evidencia de tu cuenta...",
    ...
    "followed_watchlist": "👁 Actualizando seguimiento de ideas...",
...
async def run_action(action: str, context: ContextTypes.DEFAULT_TYPE, chat_id: int) -> None:
    dispatch = {
        ...
        "followed_watchlist": action_followed_watchlist,
...
async def analytics_v2_handler(u: Update, c: ContextTypes.DEFAULT_TYPE) -> None:
    await _dispatch_command(u, c, "analytics_v2")
...
async def followed_watchlist_handler(u: Update, c: ContextTypes.DEFAULT_TYPE) -> None:
    await _dispatch_command(u, c, "followed_watchlist")
...
def build_app() -> Application:
    ...
    app.add_handler(CommandHandler("analytics",        analytics_v2_handler))
    app.add_handler(CommandHandler("analytics_v2",     analytics_v2_handler))
    ...
    app.add_handler(CommandHandler("seguimiento", followed_watchlist_handler))
    app.add_handler(CommandHandler("watchlist", followed_watchlist_handler))
    ...
    app.add_handler(CommandHandler("cocos_reintentar",          admin_refresh_portfolio_handler))
