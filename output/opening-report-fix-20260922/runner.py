...
async def run_opening_portfolio_report(run_type: str = "10:31_OPENING_PORTFOLIO") -> dict:
    ...
    try:
        # La apertura precede al refresh de mercado de las 10:40; esperar tambien
        # sus precios antes de valuar para no usar solo los de la rueda anterior.
        refresh = await request_portfolio_refresh(
            requester=f"scheduler:{run_type}",
            include_fills=True,
            include_market=True,
        ...
        * scrapea portfolio cada PORTFOLIO_OFFHOURS_REFRESH_SECONDS
        ...
        try:
            while self._running:
                ...
                if (
                    access_block_until > now_ts
                    and not (
                        refresh_request is not None
                        and refresh_request.bypass_cooldown
                    )
                ):
                ...
                if (
                    refresh_request is not None
                    and refresh_request.bypass_cooldown
                    and access_block_until > now_ts
                ):
                    logger.warning(
                        "Scraper loop: reintento manual autorizado durante cooldown "
                        "por requester=%s",
                        refresh_request.requester,
                    )
                    access_block_until = 0.0
